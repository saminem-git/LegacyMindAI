    ...
    <div className="h-full min-h-0 p-4 md:p-6 flex flex-col max-w-5xl mx-auto">
      <header className="shrink-0 border-b border-gray-800 pb-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-[var(--color-primary)] flex items-center justify-center"><Sparkles size={18} className="text-[var(--lm-electric-neon)]" /></div><div><h1 className="text-xl md:text-2xl font-bold text-[var(--color-primary)]">LegacyMind AI Assistant</h1><p className="text-xs text-gray-500 mt-1">Project Intelligence · {selectedApp.app_name}</p></div></div>
          <div className="flex items-center gap-2 text-[11px] text-gray-500"><ShieldCheck size={14} className="text-[var(--lm-vivid-green)]" /> Evidence-backed</div>
        </div>
        <div className="mt-3 flex items-center justify-between gap-3"><p className="text-sm text-gray-500">Ask about architecture, dependencies, risks, tests, and modernization evidence.</p><div className="inline-flex border border-gray-700 rounded-lg overflow-hidden shrink-0" aria-label="Assistant explanation mode">{(['technical', 'executive'] as const).map(option => <button key={option} type="button" onClick={() => setMode(option)} className={`text-[11px] px-3 py-1.5 ${mode === option ? 'bg-[var(--color-primary)] text-white' : 'bg-gray-900 text-gray-500 hover:text-gray-300'}`}>{option === 'technical' ? 'Technical' : 'Executive'}</button>)}</div></div>
      </header>

      <section className="flex-1 min-h-0 overflow-y-auto py-5 space-y-4" aria-live="polite">
        {messages.length === 0 && !answer && <div className="min-h-full flex flex-col justify-center"><div className="text-center max-w-xl mx-auto"><Bot size={28} className="mx-auto text-[var(--lm-vivid-green)]" /><h2 className="mt-4 text-lg font-semibold text-[var(--color-primary)]">Ask about your legacy system</h2><p className="mt-2 text-sm text-gray-500">LegacyMind can explain what the project evidence means and what deserves attention next.</p></div><div className="grid sm:grid-cols-2 gap-3 max-w-2xl mx-auto mt-8 w-full">{SUGGESTIONS.map(question => <button key={question} type="button" onClick={() => void askQuestion(question)} className="text-left text-sm text-gray-600 bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 hover:-translate-y-0.5 hover:border-[var(--lm-vivid-green)] hover:shadow-sm transition-all">{question}<span className="block text-xs text-gray-400 mt-1">Ask LegacyMind AI</span></button>)}</div></div>}
        ...
        {loading && <div className="flex items-center gap-2 text-sm text-gray-500"><Sparkles size={15} className="text-[var(--lm-vivid-green)] animate-pulse" /> Analyzing project evidence...</div>}
